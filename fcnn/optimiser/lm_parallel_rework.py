...
def rparallel_levenverg_marquardt_batchwise(
    p_init: torch.Tensor,
    ...
    synth: torch.Tensor = (
        torch.stack([sample["x"] for sample in data]).type(torch.float32).to(device)
    )
    synth = data_processor.input_encoder.transform(synth)[:, :2]
    ...
    p_init = torch.cat([torch.zeros(2, device=device), p_init])
    p_init = data_processor.input_encoder.transform(p_init)[2:]

    # build input dataset
    x_init: torch.Tensor = torch.cat([synth, p_init.repeat(n_samples, 1)], dim=1).to(
        device
    )
    ...
    p_k = data_processor.input_encoder.inverse_transform(p_k)[2:]
